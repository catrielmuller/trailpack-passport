...
passport.connect = function (req, query, profile, next) {
  ...
  app.services.FootprintService.find('passport', {
    ...
    if (!passport) {
      ...
      if(app.services['PermissionService'] && app.config.permissions.defaultRegisterRole){
        app.services.PermissionService.addRoleToUser(user, app.config.permissions.defaultRegisterRole)
      }
    ...
    // Action:   Get the user associated with the passport.
      ...
      if (query.hasOwnProperty('tokens') && query.tokens !== passport.tokens) {
        ...
        passport.profile = query.profile
      }

      // Save any updates to the Passport before moving on
      app.services.FootprintService.update('Passport', passport.id, passport) .then(passport => {
        app.services.FootprintService.find('Passport', passport.id, {populate: 'user'}) .then(passport => {
        app.services.FootprintService.find('user', passport.user.id)
        .then(user => {
        next(null, user)
    })
    .catch(next)
    ...
    if (!passport) {
      ...
      query.tokens = typeof query.tokens === 'string' ? query.tokens : JSON.stringify(query.tokens)
...
loadStrategies: (app) => {
  ...
  _.each(app.config.passport.strategies, (strategiesOptions, name) => {
    ...
    let host = 'localhost'
    if(app.config.endpoint.proxy_host){
      host = app.config.endpoint.proxy_host
    } else if(app.config.web.host){
      host = app.config.web.host
    }

    let port = '3000'
    if(app.config.endpoint.proxy_port){
      port = app.config.endpoint.proxy_port
    } else if (app.config.web.port) {
      port = app.config.web.port
    }

    let path = ''
    if(app.config.endpoint.proxy_path){
      path = app.config.endpoint.proxy_path
    }

    let prefix = ''
    if(app.config.footprints.prefix){
      prefix = app.config.footprints.prefix + '/'
    }

    const baseUrl = serverProtocol + '://' + host + ':' + port

    switch (protocol) {
      case 'oauth':
      case 'oauth2':
        options.callbackURL = url.resolve(baseUrl, path + prefix + callback)
        break
      case 'openid':
        options.returnURL = url.resolve(baseUrl, path + prefix +callback)
